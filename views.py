        ...
        if form.is_valid():
            ...
            try:
                ...
                if response.status_code == 204 or response.status_code == 200:
                    ...
                    new_github_run.save()
                    log_url = f"https://github.com/AsiaKing1103/rdgen/actions"
...
def check_for_file(request):
    ...
    if gh_run.status not in ['success', 'failure', 'cancelled', 'timed_out', 'skipped']:
        ...
        # Fallback: if exe files exist for this UUID, the build succeeded
        if gh_run.status not in ['success', 'failure', 'cancelled', 'timed_out', 'skipped']:
            exe_dir = os.path.join('exe', uuid)
            if os.path.isdir(exe_dir) and any(
                f.endswith(('.exe', '.msi')) for f in os.listdir(exe_dir)
            ):
